...
public class StringUtils {
    ...
    public  static String getStrHide(String a){
		...
		} else if (a != null && a.length() <= 3 && a.length() > 1) {
			StringBuffer yCod = new StringBuffer();
			for (int i = 0; i < a.length() - 1; i++) {
				yCod.append("*");
			}
			tmp = yCod.toString() + a.charAt(a.length() - 1);
	 ...
	 * @return 存在返回true
	 */
	public static boolean isDeniedHtml(String text){
		text= StringUtils.isNull(text).toLowerCase();//增加过滤器判断条件, 参数转化成小写, 再判断,
	...
	public static void main(String[] args) {
		System.out.println(isPhone("17910819824"));
	}
}
