...
function InitLeftMenu() {
	$.ajax({
		type: "POST",
		url: "/chenbund_shop/sysMenuInfo/querySysMenu.do?sysUserName="+sysUserName,
		data: "{}",
		dataType: "json",
				...
				if (jsons[i]["pid"] == 0) {
					...
					$.each(jsons, function (index, item) {
						...
						if (jsons[i]["innerid"] == item.pid) {
							uls += '<li><div><a target="mainFrame" style="cursor:pointer;" way="' +basePath+ item.url + '" ><span class="icon ' + item.icon + '" ></span>' + item.name + '</a></div>';
							uls += '</li>';
						}
					});
					uls += '</ul>';
